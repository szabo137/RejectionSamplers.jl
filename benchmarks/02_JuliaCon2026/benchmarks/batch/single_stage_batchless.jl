...
for out_type in DTYPES
    ...
    for N in nevent_vec
        for batch_size in batch_size_vec
            @info "Adding benchmark problem: dtype=$out_type, dim=$problem_size_arg, Neve=$N, Nbatch=$batch_size"
            ...
            __group[batch_size][N] = @benchmarkable @sb(
                ...
                sample_kernel = RejectionSamplers.sample_batchless_kernel($BACKEND, 32)
                backend = get_backend($SAMPLER)
                target = target_distribution($SAMPLER)
                proposal = proposal_distribution($SAMPLER)
                max_val = maximum_value($SAMPLER)
            end
        end
    end
end
